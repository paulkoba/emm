...
static Value buildBuiltinFloatingPointBinaryOp(llvm::IRBuilder<>& builder, Value lhs, Value rhs, TokenType op) {
    ...
    switch (op) {
        ...
        case TOK_MODULO:
            ...
            return {nullptr, nullptr};
        case TOK_ASSIGN:
            return {builder.CreateStore(rhs.getValue(), llvm::dyn_cast<llvm::LoadInst>(lhs.getValue())->getPointerOperand()), lhs.getType()};
        default:
...
Value buildBuiltinBinaryOp(llvm::IRBuilder<>& builder, Value lhs, Value rhs, TokenType op) {
    if(lhs.getType()->getName() != rhs.getType()->getName()) {
        compilationError("Cannot perform binary operation on types " + lhs.getType()->getName() + " and " + rhs.getType()->getName());
        return {};
    }
