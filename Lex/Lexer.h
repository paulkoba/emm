...
class Lexer {
   ...
   public:
	...
	[[nodiscard]] bool isEOF() const { return current >= bufferEnd; }

	[[nodiscard]] std::int64_t getLine() const { return line; }
	...
	[[nodiscard]] NumericToken peekNumber() const {
		...
		if (literalStart != nullptr) {
			return {llvm::StringRef(start, end - start), llvm::StringRef(literalStart, literalEnd - literalStart),
					tempLine, outputType};
		} else {
